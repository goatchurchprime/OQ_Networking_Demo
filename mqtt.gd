...
export var server = "mosquitto.doesliverpool.xyz"
...
func establishconnection(topicstatus, connectmsg, deadmsg, topicsubs):
	if client != null:
		disconnect_from_server()
	yield(get_tree().create_timer(2), "timeout")
	if deadmsg:
		set_last_will(topicstatus, deadmsg, true)
	if yield(connect_to_server(), "completed"):
		publish(topicstatus, connectmsg, true)
		if topicsubs:
			print("mqtt subscribe: ", topicsubs)
			subscribe(topicsubs)
	# should set a watchdog to keep this connection open
...
func _ready():
	...
	if client_id == "":
		...
		if true or uniqstring == "":
			uniqstring = uniqstring+"_"+str(randi())
		client_id = "u"+uniqstring
...
func connect_to_server(clean_session=true):
	...
	print("MQTT Connecting to %s:%s" % [self.server, self.port])
...
func disconnect_from_server():
	self.client.put_u16(0x00E0)
	self.client.disconnect_from_host()
	$Timer.stop()
	
func ping():
	self.client.put_u16(0x00C0)
